...
function ContestPage() {
  ...
  const tabs = ["All Contests", "Live", "Upcoming", "Past"];

  const filteredContests = useMemo(() => {
    switch (activeTab) {
      case "Live":
        return mockContests.filter(c => c.status === "ongoing");
      case "Upcoming":
        return mockContests.filter(c => c.status === "upcoming");
      case "Past":
        return mockContests.filter(c => c.status === "ended");
      default:
        return mockContests;
    }
  }, [activeTab]);
      ...
      <div className="relative z-10 mx-auto max-w-5xl px-4 pt-12">
        ...
        <div className="flex flex-wrap gap-4 mb-12">
          {tabs.map(tab => (
            <button
              ...
              className={`brutal-border px-6 py-3 font-heading font-black uppercase tracking-wider transition-all hover:-translate-y-1 hover:translate-x-1 ${
                activeTab === tab 
                  ? 'bg-primary text-black brutal-shadow-sm shadow-white' 
                  : 'bg-black text-white hover:bg-white hover:text-black border-dashed'
              }`}
            >
              {tab === "Live" && activeTab === tab && (
                <span className="inline-block h-2 w-2 rounded-full bg-black animate-pulse mr-2 align-middle"></span>
              )}
              {tab}
            </button>
          ))}
        ...
        <div className="space-y-8 min-h-[400px]">
          {filteredContests.length === 0 ? (
            <div className="brutal-border bg-black/40 p-12 text-center text-muted-foreground font-mono uppercase tracking-widest italic">
              No contests found in this sector.
            </div>
          ) : (
                  ...
                  <div className="flex flex-col md:items-end gap-4 w-full md:w-auto shrink-0 mt-2 md:mt-0">
                    ...
                    {isOngoing ? (
                       <Link 
                         ...
                         className="brutal-border bg-cyan text-black px-6 py-3 font-black uppercase text-sm hover:bg-white transition-colors brutal-shadow-sm w-full md:w-auto text-center"
                       >
                       ...
                       <Link 
                         to="/contest/$id" 
                         params={{ id: c.id.toString() }}
                         className="brutal-border bg-white text-black px-6 py-3 font-black uppercase text-sm hover:bg-primary transition-colors brutal-shadow-sm w-full md:w-auto text-center"
                       >
                         {statusLabels[c.status]}
                       </Link>
                    ) : (
                       <Link 
                         to="/contest/$id" 
                         params={{ id: c.id.toString() }}
                         className="brutal-border bg-black text-white/50 px-6 py-3 font-black uppercase text-sm hover:bg-white/10 hover:text-white transition-colors w-full md:w-auto text-center border-dashed"
                       >
                         {statusLabels[c.status]}
                       </Link>
                    )}
